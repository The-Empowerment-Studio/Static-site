...
document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch('schedule.json')
        ...
        .catch(error => {
            ...
            document.getElementById('calendar').innerHTML = '<p style="text-align: center; padding: 2rem;">Error loading schedule</p>';
        });
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            classesForDay.forEach(classInfo => {
                ...
                classElement.innerHTML = `
                    <div class="class-time">${formatTime(classInfo.startTime)}</div>
                    <div class="class-title">${classInfo.title}</div>
                    <div class="class-instructor">${classInfo.instructorName}</div>
                `;
    ...
    function renderClassDetails() {
        ...
        classes.forEach(classInfo => {
            ...
            let scheduleText = `${classInfo.dayOfWeek}s at ${formatTime(classInfo.startTime)} - ${formatTime(classInfo.endTime)}`;
            let startText = startDate ? `Classes started ${formatDate(startDate)}` : '';
    ...
    function renderWeekView() {
        ...
        weekClassesData.forEach(classInfo => {
            ...
            classCard.innerHTML = `
                <div class="week-class-day">${classInfo.dayName}, ${formatDate(classInfo.date)}</div>
                <div class="week-class-time">${formatTime(classInfo.startTime)} - ${formatTime(classInfo.endTime)}</div>
                <div class="week-class-title">${classInfo.title}</div>
                <div class="week-class-instructor">with ${classInfo.instructorName}</div>
                <div class="week-class-description">${classInfo.description}</div>
                <div class="week-class-details">
                    <div class="week-class-price">$${classInfo.price}</div>
                    <div class="week-class-age">${classInfo.minimumAgeLimit}</div>
                </div>
            `;
        ...
        if (weekClassesData.length === 0) {
            weekClasses.innerHTML = '<p style="text-align: center; color: #666; padding: 2rem;">No classes scheduled for this week.</p>';
        }
    }
